<template>
	<view class="chat">
		...
		<view class="topTabbar">
			...
			<!-- 标题 -->
			<view class="text">{{otherUser.username}}</view>
		</view>
		<scroll-view :style="{height: `${windowHeight-inputHeight - 180}rpx`}" id="scrollview" scroll-y="true"
			:scroll-top="scrollTop" class="scroll-view" @scrolltoupper="loadMoreMessage">
			<!-- 聊天主体 -->
...
<script>
	...
	export default {
	  ...
	  async	onLoad(option) {
			...
			this.acceptUserId = option.userId
			console.log(this.userinfo)
			otherUser(option.userId).then(res => {
			...
			this.msgData.acceptUserId = this.acceptUserId
			const res = await messageList(this.msgData)
			this.message = res.data.records
			this.message.forEach(item=>{
				if(item.sendUserId==this.acceptUserId)
				{
					let obj = {
						botContent: item.content,
						userContent: "",
						image: this.otherUser.avatarUrl
					}
					this.msgList.unshift(obj);
				}
				if(item.sendUserId==this.userinfo.id)
				{
					let obj = {
						botContent: "",
		...
		methods: {
			...
			openSocket() {
				...
				uni.onSocketMessage((res) => {
					...
					const acceptMessage = JSON.parse(res.data)
					let obj = {
						botContent: acceptMessage.content,
						userContent: "",
						image: this.otherUser.avatarUrl
					}
		 ...
		 async	loadMoreMessage()
		  ...
		  async	goback() {
				const res = await setMsgRead(this.acceptUserId)
				uni.reLaunch({
